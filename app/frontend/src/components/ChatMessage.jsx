...
const proseClasses = cn(
  ...
  "prose-code:rounded prose-code:bg-secondary prose-code:px-1 prose-code:py-0.5 prose-code:text-base",
  "prose-ul:my-2 prose-li:my-0.5",
);

function SystemBubble({ content }) {
  return (
    <div className="animate-fade-in rounded-xl border border-border bg-secondary/60 px-4 py-3 text-base font-medium text-muted-foreground">
      <div className="mb-1 flex items-center gap-2 text-base font-bold uppercase tracking-wide text-foreground/70">
...
function UserMessage({ content }) {
  return (
    <div className="flex animate-fade-in flex-row-reverse gap-3">
      <Avatar isUser />
      <div className="max-w-[85%] rounded-2xl border border-primary/20 bg-primary px-4 py-3 text-base font-medium leading-relaxed text-primary-foreground shadow-sm">
...
function AssistantError({ content }) {
  return (
    <div className="flex animate-fade-in gap-3">
      <Avatar isError />
      <div className="max-w-[85%] rounded-2xl border border-destructive/40 bg-destructive/10 px-4 py-3 text-base font-medium leading-relaxed text-destructive shadow-sm">
...
function AssistantMessage({ content, results }) {
  ...
  if (!hasCards) {
    return (
      <div className="flex animate-fade-in gap-3">
        <Avatar />
        <div className="max-w-[85%] rounded-2xl border border-border bg-card px-4 py-3 text-base font-medium leading-relaxed text-card-foreground shadow-sm">
          <div className={proseClasses}>
            <ReactMarkdown remarkPlugins={[remarkGfm]}>{content}</ReactMarkdown>
          </div>
        </div>
